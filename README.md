...
children for all clusters.  
2\. childrenProperties.txt: This file contains mean, standard deviation,
and variance for each cluster. 3. InferenceTestSignificantChildren.txt:
This file includes one-mismatch children that were significant in an
inference test. 4. sequences.fasta: This file includes all parents above
the threshold and the significant one-mismatch children from step 3. 5.
SvTable.txt: This count table includes all sequences from step 4. This
table can be used for taxonomic assignment and other down-stream
analyses.
